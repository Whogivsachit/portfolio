<template>
    <appLayout title="blogPage">
        <div class="flex flex-col items-center text-white py-16 mb-12 text-center bg-foreground">
            <h1 class="text-4xl font-bold mb-4">Edit an Existing Post</h1>
            <p class="text-muted">Fix or edit your posts :D</p>
        </div>
        ...
        <div class="px-4 md:px-[16rem] mx-auto">
            <div class="flex flex-col md:flex-row gap-6">
                <div class="flex flex-col items-center text-white w-full bg-foreground p-5 rounded-md">
                    <form @submit.prevent="blogEdit" class="flex flex-col w-full">
                        ...
                        <button type="submit" class="bg-accent hover:bg-blue-800 text-white rounded-md p-2 my-2">Edit Post</button>
                    </form>
                </div>
...
export default {
    ...
    async mounted() {
        const blog = await blogService.blog(this.$route.params.id);
        if(blog.status === 404) return this.$router.push('/404');
    ...
    methods: {
        async blogEdit() {
            try {
                await blogService.blogEdit(this.$route.params.id, {
                    title: this.title,
                    author: this.author,
